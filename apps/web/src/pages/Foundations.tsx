import { Card } from "@azores/ui";

const COLOR_GROUPS: ReadonlyArray<{ name: string; tokens: ReadonlyArray<[string, string]> }> = [
  {
    name: "Ocean (primary)",
    tokens: [
...
const TYPE_SCALE: ReadonlyArray<[string, string, string, number, string]> = [
...
const SPACING: ReadonlyArray<[string, number]> = [
  ["s-1", 4], ["s-2", 8], ["s-3", 12], ["s-4", 16], ["s-5", 20],
  ["s-6", 24], ["s-8", 32], ["s-10", 40], ["s-12", 48],
];

const RADII: ReadonlyArray<[string, number]> = [
  ["xs", 4], ["sm", 6], ["md", 10], ["lg", 14], ["xl", 20], ["pill", 999],
];

const SHADOWS = ["xs", "sm", "md", "lg", "xl"] as const;

export const Foundations = (): JSX.Element => (
  <div className="az-content">
    ...
    <p className="az-page-lead">
      Azores is a modern corporate design system built around volcanic basalt, Atlantic ocean,
      and warm lava. Tokens, type, and components compose into clear, dense, professional
      product surfaces.
    </p>
          ...
          <div className="az-tokens-grid">
            {group.tokens.map(([name, val]) => (
              <div key={name} className="az-token">
                <div className="az-token-swatch" style={{ background: val }} />
        ...
        <div style={{ padding: "0 24px" }}>
          {TYPE_SCALE.map(([label, size, family, weight, sample]) => (
            <div key={label} className="az-type-row">
              ...
              <div style={{ fontFamily: family, fontSize: size, fontWeight: weight, lineHeight: 1.25 }}>
                {sample}
              </div>
              <div className="az-type-meta">
                {size} · {weight}
              </div>
            </div>
          ))}
        ...
        <Card>
          <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {SPACING.map(([n, v]) => (
              ...
              >
                <span className="az-text-mono" style={{ fontSize: 12, color: "var(--az-text-3)" }}>
                  {n}
                </span>
                <div style={{ height: 8, width: v * 4, background: "var(--az-ocean-300)", borderRadius: 4 }} />
                <span className="az-text-mono" style={{ fontSize: 12, color: "var(--az-text-3)" }}>
                  {v}px
                </span>
              </div>
          ...
          >
            {RADII.map(([n, v]) => (
              ...
              >
                ...
                />
                <span className="az-text-mono" style={{ fontSize: 11, color: "var(--az-text-3)" }}>
                  {n} · {v}px
                </span>
              </div>
            ))}
